...
# Filesystem path to find the module
...
def test_calculate_custom_eval_metrics_overall(tmpdir, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location(
        MODULE_TO_TEST, PATH_TO_MODULE_TO_TEST
    )
    example_count = importlib.util.module_from_spec(spec)
    sys.modules[MODULE_TO_TEST] = example_count
    spec.loader.exec_module(example_count)
    ...
    with mock.patch("google.cloud.storage.Client") as mock_client, mock.patch(
        ...
        mock_blob.download_blob_to_file.side_effect = mock_download(
            tmpdir, PATH_TO_MODULE_TO_TEST
        )
