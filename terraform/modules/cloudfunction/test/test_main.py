...
def test_trigger_pipeline_local_yaml(
...
@mock.patch("src.main.RegistryClient")
@mock.patch("google.cloud.aiplatform.pipeline_jobs.PipelineJob")
def test_trigger_pipeline_ar_yaml_with_tag(mock_pipelinejob, mock_registryclient):

    template_path = "https://europe-west2-kfp.pkg.dev/project/repo/package_name/tag"
    enable_caching = None
    pipeline_root = "gs://my-pipeline-root/abc123"
    encryption_spec_key_name = "dummy_encryption_spec_key_name"
    project_id = "dummy_project_id"
    ...
    with mock.patch.dict(os.environ, env_vars):
        ...
        mock_pipelinejob.assert_called_with(
            ...
            enable_caching=None,
            template_path=(
                f"https://europe-west2-kfp.pkg.dev/project/repo/package_name/"
                f"{mock_registryclient.return_value.get_tag.return_value.__getitem__().__getitem__()}"  # noqa
            ),
            pipeline_root=pipeline_root,
            encryption_spec_key_name=encryption_spec_key_name,
            parameter_values=payload["pipeline_parameters"],
